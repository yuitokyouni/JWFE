...
**Current runtime milestone: v1.12.8 next-period attention
feedback.** The first cross-period attention feedback layer in
public FWE. Every period writes one
...
period N's* — proving the cross-period feedback loop closed.

Per-period record count moves from 71 to 79 (period 0) / 81
(period 1+); per-run window widens from `[284, 316]` to
`[316, 364]`. Default-fixture `living_world_digest` moves from
`2c748aa6...` to
`3002a499df6aff5c37628df5f14fbb3186481b276fab36a4fe2f13a89c5feeff`
by design — pinned in a regression test. With v1.12.8 shipped,
the living reference world has a closed cross-period feedback
loop: what an actor saw and concluded in period N changes what
it attends to in period N+1.
...
- [docs/test_inventory.md](japan-financial-world/docs/test_inventory.md)
  — 2725 tests grouped by component (444 v0 + 188 v1.0–v1.7 + 2093 post-v1.7)
...
Expected: `2725 passed` at the latest commit (444 v0 + 188 v1
