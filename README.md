...
**Current runtime milestone: v1.12.7 attention-conditioned
mechanism integration.** The orchestrator-integration milestone
that closes the v1.12.4 → v1.12.6 sequence. The per-period
valuation phase and per-period bank credit review phase in
...
**three mechanisms end-to-end** — investor intent (since
v1.12.4), valuation lite (new in v1.12.7), and bank credit
review lite (new in v1.12.7). Per-period record count and
per-run window unchanged from v1.12.4 / v1.12.5 / v1.12.6;
default-fixture `living_world_digest` moves from `d6b25704...`
to `2c748aa6e37b679d9d52984e7f2c252d434e6a2192f7fa58b71866e59f54b709`
by design (the new helpers stamp `attention_conditioned` /
`context_frame_id` / `context_frame_status` /
`context_frame_confidence` plus a `watch_label` for the credit
review onto the produced records, and those flow into the
canonical view's `valuation_added` / `signal_added` payloads).
The new digest is pinned in a regression test.
...
- [docs/test_inventory.md](japan-financial-world/docs/test_inventory.md)
  — 2613 tests grouped by component (444 v0 + 188 v1.0–v1.7 + 1981 post-v1.7)
...
Expected: `2613 passed` at the latest commit (444 v0 + 188 v1
