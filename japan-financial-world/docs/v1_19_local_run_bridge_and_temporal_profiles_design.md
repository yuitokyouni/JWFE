...
| **v1.19.2**   | **`examples/reference_world/export_run_bundle.py`** — CLI module runnable as both `python -m examples.reference_world.export_run_bundle …` and `python examples/reference_world/export_run_bundle.py …`; composes the v1.17.2 regime-comparison driver (`run_living_reference_world` per regime preset on a fresh kernel) with the v1.19.1 `world.run_export` infrastructure; **CLI export only** — no monthly profile, no scenario application wiring, no UI bridge. Closed-set CLI vocabularies: `--profile` executable in v1.19.2 = `quarterly_default` only (designed-but-not-executable: `monthly_reference` / `scenario_monthly` / `daily_display_only` / `future_daily_full_simulation` — exits non-zero with stderr `"profile '<profile>' is designed but not executable in v1.19.2"`); `--regime` one of the v1.11.2 presets (`constructive` / `mixed` / `constrained` / `tightening`); `--scenario` defaults to `none_baseline` (other v1.18.4 selector labels exit non-zero with stderr `"scenario '<scenario>' is not yet wired into the CLI in v1.19.2"`); `--out` required output path (NOT embedded in bundle); `--indent` default 2; `--quiet` suppresses success line. v1.19.3.1 reconciliation extended `--profile` executable set with `monthly_reference`; the deferred labels remain `scenario_monthly` / `daily_display_only` / `future_daily_full_simulation`. +20 tests in `tests/test_run_export_cli.py`. | **Shipped (4410 tests)**       |
| **v1.19.3**   | **`monthly_reference` run profile + `world/information_release.py`** — `InformationReleaseCalendar` book + `ScheduledIndicatorRelease` + `InformationArrivalRecord` + nine closed-set frozensets (`RELEASE_CADENCE_LABELS` 8 entries / `INDICATOR_FAMILY_LABELS` 12 entries / `RELEASE_IMPORTANCE_LABELS` 5 entries / `JURISDICTION_SCOPE_LABELS` 4 / `ARRIVAL_STATUS_LABELS` 5 / `REASONING_MODE_LABELS` 4 / `REASONING_SLOT_LABELS` 4 / `STATUS_LABELS` 6 / `VISIBILITY_LABELS` 3); v1.19.3 hard-naming-boundary `FORBIDDEN_INFORMATION_RELEASE_FIELD_NAMES` frozenset (composes the v1.18.0 actor-decision tokens with the v1.19.3 Japan-real-data tokens — `real_indicator_value` / `cpi_value` / `gdp_value` / `policy_rate` / `real_release_date` / `boj` / `fomc` / `ecb`); three new `RecordType` enum values (`INFORMATION_RELEASE_CALENDAR_RECORDED` / `SCHEDULED_INDICATOR_RELEASE_RECORDED` / `INFORMATION_ARRIVAL_RECORDED`); kernel wired with `information_releases: InformationReleaseBook` empty by default; `run_living_reference_world(..., profile=...)` accepts `quarterly_default` (default, byte-identical to v1.19.1) and `monthly_reference`; the monthly profile registers a default jurisdiction-neutral synthetic calendar (12 months, 7 indicator families) and emits one `InformationArrivalRecord` per scheduled release per month — bounded at **3-5 arrivals per month, total 51** for the default fixture (within the [36, 60] design budget); `LivingReferencePeriodSummary` extended with `scheduled_release_ids` / `information_arrival_ids` (empty for `quarterly_default`); arrivals carry `reasoning_mode = "rule_based_fallback"` / `reasoning_slot = "future_llm_compatible"` / the v1.19.0 default boundary-flag set verbatim; `monthly_reference` produces a deterministic `living_world_digest` pinned at **`75a91cfa35cbbc29d321ffab045eb07ce4d2ba77dc4514a009bb4e596c91879d`**; the `quarterly_default` digest stays byte-identical at **`f93bdf3f4203c20d4a58e956160b0bb1004dcdecf0648a92cc961401b705897c`**; +88 tests in `tests/test_information_release.py` + +16 tests in `tests/test_living_reference_world.py` and `tests/test_living_reference_world_performance_boundary.py`. | **Shipped (4494 tests)**       |
| **v1.19.4**   | **UI local run bundle loader (read-only)** — adds **Load local bundle** to the top ribbon of [`examples/ui/fwe_workbench_mockup.html`](../examples/ui/fwe_workbench_mockup.html); `<input type="file">` + `FileReader.readAsText` + `JSON.parse`; validates the v1.19.1 top-level key set + the v1.19.0 default 8-flag boundary-flag block; accepts `quarterly_default` / `monthly_reference`; rejects `scenario_monthly` / `daily_display_only` / `future_daily_full_simulation` with `bundle profile '<profile>' is not loadable in v1.19.4 static UI`; renders user-loaded values via `textContent` only (never `innerHTML`); caps the rendered ledger excerpt at 20 rows; new `current_data_source` label tracks `inline_fixture` / `sample_manifest` / `local_bundle`; new **Local run bundle** card on the Inputs tab surfaces the loaded bundle's labels + a profile badge; `monthly_reference` bundles render the v1.19.3 `metadata.information_arrival_summary` (calendar / scheduled-release / arrival counts + per-`indicator_family` / per-`importance` / per-`arrival_status` histograms); preserves the v1.17.4 / v1.18.4 no-jump discipline; in-page `Validate` gains six new audit checks; **no engine execution from the UI**, **no backend**, **no file-system write**, **no fetch / XHR / network I/O**; static HTML / CSS / JS only — runnable directly under `file://`; the optional `127.0.0.1` stub local server bridge remains deferred (FastAPI / Flask / `http.server` only — never Rails); no test count change (UI-only milestone). | **Shipped (4522 tests)**       |
| **v1.19.last**| **Local Run Bundle and Monthly Reference freeze** (docs-only) — single-page reader-facing summary in [`docs/v1_19_local_run_bundle_and_monthly_reference_summary.md`](v1_19_local_run_bundle_and_monthly_reference_summary.md); §128.20 in `docs/world_model.md`; v1.19.last release-readiness snapshot in `RELEASE_CHECKLIST.md`; v1.19.last freeze-pin section in `docs/performance_boundary.md`; v1.19.last `test_inventory.md` header note; v1.19.last addendum in `examples/reference_world/README.md`; v1.19.last cross-link in `docs/fwe_reference_demo_design.md` and `examples/ui/README.md`; no new code, no new tests, no new ledger event types; test count = **4522 / 4522**, per-period record count (`quarterly_default`) = **108 / 110**, per-run window = **`[432, 480]`**, `living_world_digest` (`quarterly_default`) = **`f93bdf3f4203c20d4a58e956160b0bb1004dcdecf0648a92cc961401b705897c`** (unchanged from v1.18.last by design across the entire v1.19 sequence); `monthly_reference` `living_world_digest` = **`75a91cfa35cbbc29d321ffab045eb07ce4d2ba77dc4514a009bb4e596c91879d`** with **51 arrivals / 12 months**. | **Shipped (4522 tests)** |
...
(**shipped**). v1.19.2 lands the CLI exporter (**shipped**) —
`python -m examples.reference_world.export_run_bundle …`
wires the v1.17.2 regime-comparison reader to the v1.19.1
`build_run_export_bundle` / `write_run_export_bundle`
helpers. v1.19.3 lands the `monthly_reference` profile *and*
the `InformationReleaseCalendar` layer (**shipped** —
[`world/information_release.py`](../world/information_release.py)
ships nine closed-set frozensets, three immutable record
shapes, and an append-only `InformationReleaseBook`; the
default `monthly_reference` fixture produces 51 synthetic
information arrivals across 12 months with the pinned digest
`75a91cfa…91879d`; the canonical `quarterly_default` digest
stays byte-identical at `f93bdf3f…b705897c` because the new
book is empty by default). v1.19.3.1 reconciliation extends
the v1.19.2 CLI exporter to also emit `monthly_reference`
bundles. v1.19.4 lands the UI local bundle loader
(**shipped** — `<input type="file">` + `FileReader` +
`JSON.parse`; read-only; validates the v1.19.1 schema and
the v1.19.0 default 8-flag boundary block; accepts
`quarterly_default` / `monthly_reference`, rejects the three
deferred profile labels; renders into the new Local-run-bundle
card on Inputs and surfaces the v1.19.3
`information_arrival_summary` for monthly bundles). The
optional `127.0.0.1` stub local server bridge stays deferred —
the CLI + read-only UI loader cover the headline workflow.
v1.19.last freezes.
