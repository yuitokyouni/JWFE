...
- **Per-milestone roadmap.** v1.18.0 design (this); v1.18.1 `ScenarioDriverTemplate` storage + 20 default templates; v1.18.2 `apply_scenario_driver(...)` helper (rule-based-fallback only — `reasoning_mode` binding); v1.18.3 scenario report / causal timeline integration with the v1.17.2 / v1.17.3 surfaces; v1.18.4 UI scenario selector mock (no-jump-disciplined, fixture-switching only — Python engine NOT invoked from the UI); v1.18.last freeze (docs-only).
