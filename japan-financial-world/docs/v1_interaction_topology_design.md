...
| **v1.8.13 Investor / Bank Review Routines** | Code: two narrow consumer routines on the diagonal Investors → Investors and Banking → Banking self-loops (channel types `investor_review_channel` / `bank_credit_review_channel`) that read `SelectedObservationSet` records through `RoutineEngine`, persist one `RoutineRunRecord` each, and emit one synthetic `investor_review_note` / `bank_review_note` `InformationSignal`. Closes the endogenous chain *corporate report → menus → heterogeneous selections → review run records → review notes* end-to-end. **Audit artifacts only — no buy / sell / lend / cover / valuation / price behavior.** **Shipped** as `world/reference_reviews.py` (32 tests; 1257 → 1289 total). | Yes (two routines) |
