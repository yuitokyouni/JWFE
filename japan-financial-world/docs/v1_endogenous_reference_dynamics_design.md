...
| **v1.8.5 AttentionProfile / ObservationMenu / SelectedObservationSet** | The §44 attention layer that v1.8.4 originally bundled but was split out. Records + view builder + the corresponding `attention_*` ledger event types. | Yes (kernel) |
| **v1.8.6 Routine engine (execution)** | Schedule-and-fire wiring that consumes routines + attention to produce `RoutineRunRecord` entries automatically. | Yes (kernel + engine) |
