...
| **v1.22 Institutional Investor Mandate / Benchmark Pressure** | benchmark / peer / mandate constraints on the v1.15.5 / v1.16.2 investor-intent layer; bounded synthetic mandate / benchmark id vocabulary. Decoupled from the stress composition layer; may ship in parallel. | Roadmap candidate.                                                                  |
| **v1.22 manual-annotation-only stress interaction layer** *(optional)* | a `manual_annotation`-only annotation layer over the v1.21.3 multiset readout: a human reviewer types in `interaction_label = amplify` / `dampen` / `offset` / `coexist` after reading the citations, with their own analyst id and timestamp on the annotation record, citing explicit evidence from a specific `(period, context_surface, active_stress_step_id_set)` cell. **MUST NEVER be inferred by a helper, a classifier, a closed-set rule table, an LLM, or any other automated layer.** MUST NOT replace the multiset readout. | Optional candidate. Not currently planned. Would require a fresh design pin. |
| **v1.21.3 markdown summary UI strip** *(optional)*       | a small static UI strip that renders the v1.21.3 markdown summary verbatim in the v1.20.5 static workbench's Universe tab. **Read-only static rendering only — no engine execution from the browser, no backend, no fetch / XHR.** | Optional candidate. Not currently planned. The v1.21.3 markdown surface is byte-stable, so this would be a thin static-rendering pass with no Python changes. |
