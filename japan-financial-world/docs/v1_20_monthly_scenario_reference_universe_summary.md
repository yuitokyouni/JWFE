...
| **v1.22 Static UI Stress Readout Reflection**            | reflects the v1.21.3 read-only multiset readout in the v1.20.5 static workbench's existing Universe tab. v1.22.1: new descriptive-only `stress_readout` payload section on `RunExportBundle` (empty-by-default; pre-v1.22 digests preserved byte-identical). v1.22.2: new "Active Stresses" strip above the sector heatmap with 12 monthly cells, family tokens, `resolved / total` counter, `partial: N` text badge, and a details panel for context-surface / shift-direction / scenario-family multisets and citation lists. **No new tab. No bar height. No score. No `impact` / `outcome` / `risk score` / `prediction` / `recommendation` / `magnitude` wording. No backend; static JSON loaded via `<input type="file">` + `FileReader` only.** | **Design scoped at v1.22.0** (docs-only — see [`v1_22_static_ui_stress_readout_reflection.md`](v1_22_static_ui_stress_readout_reflection.md) and `world_model.md` §131). v1.22.0 ships zero runtime / UI code; v1.22.1 / v1.22.2 are the implementation sub-milestones. |
| **v1.23 Institutional Investor Mandate / Benchmark Pressure** | benchmark / peer / mandate constraints on the v1.15.5 / v1.16.2 investor-intent layer; bounded synthetic mandate / benchmark id vocabulary. Decoupled from the stress composition layer; may ship in parallel. | Roadmap candidate.                                                                  |
| **manual-annotation-only stress interaction layer** *(optional, post-v1.22)* | a `manual_annotation`-only annotation layer over the v1.21.3 multiset readout: a human reviewer types in `interaction_label = amplify` / `dampen` / `offset` / `coexist` after reading the citations, with their own analyst id and timestamp on the annotation record, citing explicit evidence from a specific `(period, context_surface, active_stress_step_id_set)` cell. **MUST NEVER be inferred by a helper, a classifier, a closed-set rule table, an LLM, or any other automated layer.** MUST NOT replace the multiset readout. | Optional candidate. Not currently planned. Would require a fresh design pin. |
