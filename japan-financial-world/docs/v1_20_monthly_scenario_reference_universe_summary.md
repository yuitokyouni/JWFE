...
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| **v1.21 Stress Composition Layer**                       | thin orchestrator + read-only multiset readout over the existing v1.18 scenario-driver application chain. Reuses v1.18.1 `ScenarioDriverTemplate` ids verbatim; bundles them into a named, ordered `StressProgramTemplate`; applies them step-by-step via the existing v1.18.2 `apply_scenario_driver(...)` helper; emits **one program-level receipt** (`StressProgramApplicationRecord`) that *collects* the underlying `scenario_application_ids`; provides a read-only `StressFieldReadout` **multiset projection** per `(period, context_surface)`. **No interaction inference**, **no aggregate / composite / net / dominant fields**, **no `amplify` / `dampen` / `offset` labels** (deferred to v1.22+ as `manual_annotation`-only — never auto-inferred by a helper). Cardinality binding: ≤ 1 stress program per run, ≤ 3 stress steps per program, ≤ 60 v1.21 records added. **Append-only / read-only / no causality claim / no magnitudes / no price / no recommendation.** | **Design scoped at v1.21.0a** (corrected from v1.21.0 — see [`v1_21_stress_composition_layer.md`](v1_21_stress_composition_layer.md) and `world_model.md` §130.1 / §130.7). |
