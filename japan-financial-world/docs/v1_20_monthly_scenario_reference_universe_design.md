...
| v1.20.1       | `world/reference_universe.py` — `ReferenceUniverseProfile` + `GenericSectorReference` + `SyntheticSectorFirmProfile` immutable frozen dataclasses + closed-set vocabularies + `ReferenceUniverseBook` append-only storage + new `RecordType` enum values (`REFERENCE_UNIVERSE_PROFILE_RECORDED` / `GENERIC_SECTOR_REFERENCE_RECORDED` / `SYNTHETIC_SECTOR_FIRM_PROFILE_RECORDED`); kernel wired with `reference_universe: ReferenceUniverseBook` empty by default; `FORBIDDEN_REFERENCE_UNIVERSE_FIELD_NAMES` frozenset; **storage only**, no run profile yet. Trip-wire tests pin: empty book does not move `quarterly_default` / `monthly_reference` digests; module text contains no real-issuer / real-taxonomy tokens; sensitivity matrix byte-identical across runs. | Planned                 |
| v1.20.2       | `world/scenario_schedule.py` — `ScenarioSchedule` + `ScheduledScenarioApplication` immutable frozen dataclasses + `ScenarioScheduleBook` append-only storage + new `RecordType` enum value (`SCHEDULED_SCENARIO_APPLICATION_RECORDED`); kernel wired with `scenario_schedule: ScenarioScheduleBook` empty by default; **storage only**, schedules are not yet executed. Trip-wire tests pin: empty book does not move canonical digests; schedules carry the v1.18.0 audit shape; no actor-decision field. | Planned                 |
| v1.20.3       | `scenario_monthly_reference_universe` run profile in `world/reference_living_world.py` — composes the v1.19.3 monthly cadence + v1.19.3 information arrivals + the new v1.20.1 universe (11 sectors / 11 firms / 4 investors / 3 banks) + the v1.20.2 scheduled scenario applications + the §8 scenario-to-sector impact map. Each scheduled month emits one `ScenarioDriverApplicationRecord` per scheduled application + per-firm `ScenarioContextShiftRecord` instances per the impact rules. **Append-only.** Pinned digest for the default test fixture (one credit_tightening at month 4). +N tests covering the §6 performance budget and the §12 validation requirements. | Planned                 |
| v1.20.4       | `examples/reference_world/export_run_bundle.py` extension — accepts `--profile scenario_monthly_reference_universe`. Adds the §11 v1.20-specific bundle sections (`sector_summary` / `firm_summary` / `scenario_application_summary` / `scenario_context_shift_summary` / `sector_impact_summary` / `monthly_timeline`). Bumps `ledger_excerpt` cap from 20 to 30 for the richer universe. The CLI continues to reject `scenario_monthly` / `daily_display_only` / `future_daily_full_simulation` (v1.19.3.1 discipline preserved). +N CLI tests. | Planned                 |
