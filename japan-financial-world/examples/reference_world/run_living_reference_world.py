...
def _print_trace(result: LivingReferenceWorldResult) -> None:
    ...
    for idx, ps in enumerate(result.per_period_summaries, start=1):
        print(
            ...
            f"market_intents={len(ps.investor_market_intent_ids)} "
            f"aggregated_interest={len(ps.aggregated_market_interest_ids)} "
            f"market_pressure={len(ps.indicative_market_pressure_ids)} "
    ...
    print(
        ...
        "path -> investor market intent -> aggregated market "
        "interest -> indicative market pressure. No price "
        "formation, no trading, no lending decisions, no "
        "covenant enforcement, no contract or constraint "
        "mutation, no firm financial statement updates, no "
        "accounting values, no canonical-truth valuation, no "
        "investment advice, no voting execution, no proxy "
        "filing, no public-campaign execution, no "
        ...
        "optimisation, no real leverage / D/E / WACC, no "
        "PriceBook mutation, no order book, no order imbalance, "
        "no bid / ask, no target price, no expected return."
    )
